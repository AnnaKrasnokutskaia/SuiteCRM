package wrappers;

import org.openqa.selenium.By;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.WebElement;
import org.openqa.selenium.support.ui.ExpectedConditions;
import org.openqa.selenium.support.ui.WebDriverWait;

import java.time.Duration;

/**
 * Wrapper для textarea.
 * Textarea ищется по видимому label строки формы.
 */
public class Textarea {

    private final WebDriverWait wait;
    private final String label;

    /**
     * Создаёт wrapper для textarea с указанным label.
     */
    public Textarea(WebDriver driver, String label) {
        this.wait = new WebDriverWait(driver, Duration.ofSeconds(15));
        this.label = label;
    }

    /**
     * Находит textarea, очищает её и вводит текст.
     */
    public void write(String text) {
        WebElement textarea = wait.until(ExpectedConditions.elementToBeClickable(textareaByLabel()));
        textarea.clear();
        textarea.sendKeys(text);
    }

    /**
     * Возвращает локатор textarea внутри строки формы с нужным label.
     */
    private By textareaByLabel() {
        return By.xpath(editFieldRowByLabel(label) + "//textarea[1]");
    }

    /**
     * Собирает XPath строки формы edit-view-row-item по видимому названию поля.
     */
    private String editFieldRowByLabel(String label) {
        return "//form[@id='EditView']" +
                "//div[contains(@class, 'edit-view-row-item')" +
                " and .//div[contains(@class, 'label')" +
                " and contains(normalize-space(.), '" + label + "')]]";
    }
}
